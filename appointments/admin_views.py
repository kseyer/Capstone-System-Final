...
def admin_dashboard(request):
    ...
    # Get statistics
    # Total appointments = Pending + Confirmed + Cancelled (excluding products)
...
def admin_appointments(request):
    ...
    # Calculate statistics BEFORE filtering (for the cards)
    total_appointments = Appointment.objects.filter(
        Q(service__isnull=False) | Q(package__isnull=False) | Q(product__isnull=False)
    ).count()
...
def admin_confirm_appointment(request, appointment_id):
    ...
    from services.utils import send_attendant_assignment_sms
    ...
    if appointment.status in ['scheduled', 'pending']:
        ...
        # Send SMS/in-app notification to Patient with full appointment details
        confirmation_message = f"Your appointment has been confirmed. {appointment.get_service_name()} on {appointment.appointment_date} at {appointment.appointment_time}. Attendant: {appointment.attendant.first_name} {appointment.attendant.last_name}."
        ...
        # Send SMS/in-app notification to Attendant with full patient + appointment details
        try:
            ...
            if attendant_user:
                ...
                attendant_message = f"You have been assigned a new appointment. Patient: {appointment.patient.get_full_name()}. {appointment.get_service_name()} on {appointment.appointment_date} at {appointment.appointment_time}."
                Notification.objects.create(
                    type='confirmation',
                    appointment_id=appointment.id,
            ...
            pass  # Don't fail if attendant notification fails
        
        if sms_result['success']:
            messages.success(request, f'Appointment for {appointment.patient.full_name} has been confirmed. SMS sent to patient and attendant.')
        else:
        ...
        messages.error(request, 'Only scheduled or pending appointments can be confirmed.')
...
def admin_inventory(request):
    ...
    # Get product purchase history (last 50 records)
    product_history = ProductHistory.objects.select_related('product', 'performed_by').order_by('-created_at')[:50]
...
def admin_export_analytics(request):
    ...
    analytics_service = AnalyticsService()
    export_type = request.GET.get('type', 'overview')  # overview, revenue, patients, services
    
    # Create the HttpResponse object with CSV header
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = f'attachment; filename="analytics_export_{export_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
    ...
    if export_type == 'overview':
        ...
        writer.writerow(['Total Revenue', business_overview.get('total_revenue', 0)])
        writer.writerow(['Total Appointments', business_overview.get('total_appointments', 0)])
        writer.writerow(['Total Patients', business_overview.get('total_patients', 0)])
        writer.writerow(['Average Revenue per Appointment', business_overview.get('avg_revenue_per_appointment', 0)])
        
    elif export_type == 'revenue':
        revenue_analytics = analytics_service.get_revenue_analytics()
        writer.writerow(['Period', 'Revenue'])
        for period, revenue in revenue_analytics.get('revenue_by_period', {}).items():
            writer.writerow([period, revenue])
            
    elif export_type == 'patients':
        patient_analytics = analytics_service.get_patient_analytics()
        writer.writerow(['Metric', 'Value'])
        writer.writerow(['Total Patients', patient_analytics.get('total_patients', 0)])
        writer.writerow(['New Patients', patient_analytics.get('new_patients', 0)])
        writer.writerow(['Returning Patients', patient_analytics.get('returning_patients', 0)])
        
    elif export_type == 'services':
        service_analytics = analytics_service.get_service_analytics()
        writer.writerow(['Service', 'Bookings', 'Revenue'])
        for service_data in service_analytics.get('top_services', []):
            writer.writerow([
                service_data.get('name', ''),
                service_data.get('bookings', 0),
                service_data.get('revenue', 0)
            ])
...
def admin_update_stock(request, product_id):
    ...
    if request.method == 'POST':
        action = request.POST.get('action')  # 'add', 'set', or 'minus'
        ...
        if action == 'add':
            ...
            # Deduct stock for on-premises purchase
            if product.stock < quantity:
                messages.error(request, f'Cannot deduct {quantity} units. Current stock is only {product.stock} units.')
                return redirect('appointments:admin_inventory')
            product.stock -= quantity
            # Log the purchase
            ProductHistory.objects.create(
                product=product,
                quantity=quantity,
                purchase_type='on_premises',
                performed_by=request.user,
                notes=notes or f'On-premises purchase - {quantity} units'
            )
            messages.success(request, f'Deducted {quantity} unit{"s" if quantity != 1 else ""} from {product.product_name} (on-premises purchase). New stock: {product.stock}')
        else:
        ...
        if action == 'add':
            ProductHistory.objects.create(
                product=product,
                quantity=quantity,
                purchase_type='restock',
                performed_by=request.user,
                notes=notes or f'Restocked - Added {quantity} units'
            )
