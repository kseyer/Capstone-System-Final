...
def get_available_attendants(selected_date=None, selected_time=None):
    ...
    for user in attendant_users:
        ...
        # Use filter().first() to handle duplicate attendants with same name
    ...
    if selected_date and selected_time:
        try:
            ...
            # Get attendants who already have appointments at this time
            booked_attendant_ids = Appointment.objects.filter(
                appointment_date=selected_date,
                appointment_time=selected_time,
                status__in=['scheduled', 'pending', 'confirmed']
            ).values_list('attendant_id', flat=True)
            
            # Filter attendants by availability
            available_attendant_ids = []
            for attendant in all_attendants:
                # Skip if attendant is already booked at this time
                if attendant.id in booked_attendant_ids:
                    continue
...
def book_service(request, service_id):
    ...
    if request.method == 'POST':
        ...
        if appointment_date and appointment_time:
            ...
            # Check for existing appointments at the same time slot with the same attendant
            ...
            # Only 1 patient per attendant per time slot
            if existing_appointment:
                messages.error(request, f'This attendant ({attendant.first_name} {attendant.last_name}) is already booked at this time. Please choose another time or select a different attendant.')
                context = {
                    'service': service,
                    'attendants': available_attendants,
                    'selected_date': appointment_date,
            ...
            # Create appointment with 'scheduled' status (pending staff confirmation)
            ...
            # Send alert + appointment details to Staff (admin/owner)
            Notification.objects.create(
                type='appointment',
                appointment_id=appointment.id,
                title='New Appointment Scheduled',
                message=f'New appointment scheduled: {appointment.patient.get_full_name()} - {appointment.get_service_name()} on {appointment.appointment_date} at {appointment.appointment_time}. Attendant: {attendant.first_name} {attendant.last_name}. Transaction ID: {transaction_id}.',
                patient=None  # Staff notification
            )
            
            # Send SMS + in-app notification to Patient
            patient_message = f"Thank you! Your appointment has been scheduled and is now being processed. Please wait for confirmation as our staff will personally review the details. A final confirmation with complete information will be sent shortly. Transaction ID: {transaction_id}"
            ...
            messages.success(request, f'Appointment scheduled successfully! Please wait for staff confirmation. Transaction ID: {transaction_id}')
    ...
    # Check if patient is first-time (has no completed appointments)
    is_first_time_patient = not Appointment.objects.filter(
        patient=request.user,
        status='completed'
    ).exists()
    ...
    if request.method == 'POST':
        ...
        if appointment_date and appointment_time:
            ...
            if quantity > product.stock:
                messages.error(request, f'Requested quantity ({quantity}) exceeds available stock ({product.stock}). Please adjust your order.')
                context = {
                    'product': product,
                }
                return render(request, 'appointments/book_product.html', context)
            ...
            # Only 1 patient per attendant per time slot
            if existing_appointment:
                messages.error(request, f'This attendant ({attendant.first_name} {attendant.last_name}) is already booked at this time. Please choose another time.')
                context = {
                    'product': product,
                }
                return render(request, 'appointments/book_product.html', context)
            ...
            initial_status = 'confirmed' if product.stock >= quantity else 'pending'
            ...
            HistoryLog.objects.create(
                ...
                item_type='appointment',
                item_id=appointment.id,
                item_name=f"{product.product_name} (Qty: {quantity}) - {request.user.get_full_name()}",
                performed_by=request.user,
                details={
                    'appointment_id': appointment.id,
                    'patient': request.user.get_full_name(),
            ...
            # Create notification with pre-order type
            Notification.objects.create(
                type='pre-order',
                appointment_id=appointment.id,
                title='Product Pre-Ordered',
                message=f'Your {product.product_name} (Quantity: {quantity}) has been pre-ordered for pickup on {appointment_date} at {appointment_time}. Total: ₱{total_amount:.2f}. Transaction ID: {transaction_id}',
                patient=request.user
            )
            ...
            Notification.objects.create(
                ...
                message=f'Product pre-order: {request.user.get_full_name()} - {product.product_name} (Quantity: {quantity}, Total: ₱{total_amount:.2f}) on {appointment_date} at {appointment_time}. Status: {initial_status}.',
    ...
    if request.method == 'POST':
        ...
        if appointment_date and appointment_time:
            ...
            Notification.objects.create(
                type='appointment',
                appointment_id=appointment.id,
                title='New Package Appointment Scheduled',
                message=f'New package appointment scheduled: {request.user.get_full_name()} - {package.package_name} on {appointment_date} at {appointment_time}. Attendant: {attendant.first_name} {attendant.last_name}. Transaction ID: {transaction_id}.',
                patient=None  # Staff notification
            )
            ...
            messages.success(request, f'Package appointment scheduled successfully! Please wait for staff confirmation. Transaction ID: {transaction_id}')
            return redirect('appointments:my_appointments')
        else:
            messages.error(request, 'Please fill in all required fields.')
