...
def attendant_appointment_detail(request, appointment_id):
    ...
    # Get today's date for Treatment Done button check
    today = timezone.now().date()
    is_appointment_today = appointment.appointment_date == today
    
    context = {
        'appointment': appointment,
        'feedback': feedback,
        'attendant_feedback': attendant_feedback,
        'is_appointment_today': is_appointment_today,
    }
...
def attendant_complete_appointment(request, appointment_id):
    ...
    # Check if today == appointment date (Treatment Done button rule)
    from django.utils import timezone
    today = timezone.now().date()
    if appointment.appointment_date != today:
        messages.error(request, f'Treatment can only be marked as done on the appointment date ({appointment.appointment_date}). Today is {today}.')
        return redirect('attendant:appointment_detail', appointment_id=appointment_id)
    
    if appointment.status in ['scheduled', 'pending', 'confirmed']:
